...
public class WallSelectionHandler : SelectionHandler
{
    [SerializeField]
        ...
        if (!menuManager.IsMenuActive() && Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out hit, 100))
            {
                ...
                if (int.TryParse(hit.transform.name.Substring(0, 1), out int value) )
                {
                    ...
                    // If a wall has been added to the selected walls handle the World UI component
    ...
    // Obtain the active child (belonging to one material) of this wall
    ...
    // Obtain the original material based on the name of the GameObject
